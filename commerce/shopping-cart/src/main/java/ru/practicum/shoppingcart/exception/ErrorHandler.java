...
public class ErrorHandler extends GlobalErrorHandler {
    ...
    public ApiError handleProductAlreadyInWarehouse(NotAuthorizedUserException e) {
    ...
    public ApiError handleProductAlreadyInWarehouse(NoProductsInShoppingCartException e) {
        log.error("Ошибка: Нет искомых товаров в корзине (NoProductsInShoppingCartException): {}", e.getMessage());
        return build(HttpStatus.BAD_REQUEST, "Ошибка, тНет искомых товаров в корзине.", e.getMessage(),
                NoProductsInShoppingCartException.class.getSimpleName());
    }

    @ExceptionHandler(NotFoundUserException.class)
    @ResponseStatus(HttpStatus.NOT_FOUND)
    public ApiError handleProductAlreadyInWarehouse(NotFoundUserException e) {
