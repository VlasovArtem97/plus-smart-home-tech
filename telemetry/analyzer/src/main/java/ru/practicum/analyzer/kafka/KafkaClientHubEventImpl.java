...
import ru.practicum.analyzer.deserializer.HubEventAvroDeserializer;

import java.time.Duration;
import java.util.Properties;

@Component("kafkaClientHubEventImpl")
@Slf4j
public class KafkaClientHubEventImpl implements KafkaClient, AutoCloseable {

    @Value("${kafka.port}")
    private String kafkaPort;

    @Value("${kafka.consumerClientIdForAnalyzerHubEvent}")
    ...
    private void initConsumer() {
        ...
        config.put(ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG, HubEventAvroDeserializer.class.getName());
